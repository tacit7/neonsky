...
local Groups = require("neonsky.groups")
...
function M.extras()
  local Extra = require("neonsky.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/neonsky/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/neonsky/config.lua", "\n(--@class neonsky%.Config.-\n})")
